...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    # row_indices = torch.randint(0, xy_grid.shape[0], n_pixels)s
    # col_indices = torch.randint(0, xy_grid.shape[1], n_pixels)

    rand_indices = torch.randperm(xy_grid.shape[0])
    xy_grid_sub = xy_grid[rand_indices]
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    world_space_pts = camera.unproject_points(ndc_points)

    # TODO (Q1.3): Get ray origins from camera center
    ray_origin = camera.get_camera_center()
    rays_o = ray_origin.repeat(world_space_pts.shape[0], 1)
